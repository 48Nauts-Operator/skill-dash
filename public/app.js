...
function whyText(x){
 ...
 if(x.alternatives?.length)parts.push(x.alternatives.length>1?`${x.alternatives.length} other skills do the same job; this one scored best.`:'One other skill does the same job; this one scored best.');
 return parts.join(' ');
}
async function renderFind() {
 ...
 $('queue-note').hidden=!findData?.queued;if(findData?.queued)$('queue-note').innerHTML=`<b>${findData.queued} accepted</b> and queued at <code>${esc(findData.queue_path)}</code>. Tell your agent: <code>read ~/.claude/skill-dash/queue.md and install what is queued, one at a time, reading each skill first</code>. Nothing was installed by this dashboard.`;
 ...
 $('find-rows').innerHTML=rows.slice(0,findLimit).map(x=>`<tr><td><a href="https://github.com/${esc(x.r)}/blob/${esc(x.c)}/${esc(x.p)}/SKILL.md"><b>${esc(x.n)}</b></a><br><small>${esc(x.r)}</small><br><span class="preview" style="white-space:normal">${esc(x.d)}</span>${x.alternatives?.length?`<br><small>same job: ${x.alternatives.map(a=>`<a href="https://github.com/${esc(a.r)}/blob/${esc(a.c)}/${esc(a.p)}/SKILL.md">${esc(a.n)}</a> <span style="color:var(--muted)">(${esc(a.r.split('/')[0])}, gap ${(a.gap??0).toFixed(2)})</span>`).join(' · ')}</small>`:''}</td><td>${scoreTag(x.gap)}</td><td>${scoreTag(x.fit)}<br><small>conf ${Math.round(x.fit_conf*100)}%</small></td><td class="confidence-cell">${Math.round(x.covered*100)}%</td><td style="min-width:330px;max-width:420px;white-space:normal;font-size:11.5px;color:#cfc4ba;line-height:1.5">${whyText(x)}</td><td>${x.flags?.length?`<span class="flag-badge" title="${esc(x.flags.join(', '))}">⚠ ${esc(x.flags.join(', '))}</span>`:'<small>clean</small>'}${x.j!=null?`<br><small>Jev risk ${x.j}</small>`:''}${x.cp?`<br><small>${x.cp} copies elsewhere</small>`:''}${x.dupes?`<br><small>${x.dupes} identical in the cut</small>`:''}</td><td style="max-width:300px"><code style="font-size:10px;white-space:normal">git clone --depth 1 https://github.com/${esc(x.r)} /tmp/${esc(x.r.split('/')[1])} && cp -r /tmp/${esc(x.r.split('/')[1])}/${esc(x.p)} ~/.claude/skills/${esc(x.n)}</code></td><td>${x.decision==='accepted'?'<span class="accepted-tag">accepted · queued</span>':`<details class="rowmenu"><summary aria-label="Actions">···</summary><div><button data-rec="accepted" data-r="${esc(x.r)}" data-p="${esc(x.p)}" data-n="${esc(x.n)}" data-c="${esc(x.c)}">Accept<small>queue it for your agent to install</small></button><button class="danger" data-rec="rejected" data-r="${esc(x.r)}" data-p="${esc(x.p)}" data-n="${esc(x.n)}" data-c="${esc(x.c)}">Delete<small>I disagree; hide it and keep it out of future searches</small></button></div></details>`}</td></tr>`).join('')||'<tr><td colspan="8" class="empty"><strong>No search yet.</strong>Press Find skills. Nothing is written to your tree.</td></tr>';
}
$('find-more').onclick=()=>{findLimit+=5;renderFind();};
document.addEventListener('click',async e=>{const b=e.target.closest('[data-rec]');if(!b)return;const d=b.dataset;try{await api('/api/recommend/decide',{r:d.r,p:d.p,n:d.n,c:d.c,decision:d.rec});findLoadedAt='';toast(d.rec==='accepted'?`${d.n} queued for your agent. See the note above the table.`:`${d.n} hidden and excluded from future searches.`);await renderFind();}catch(err){toast(err.message,true);}});
...
document.addEventListener('click',e=>{const i=e.target.closest('th span[title]');if(i){const th=i.closest('th'),wrap=th.closest('.table-scroll');let help=wrap.nextElementSibling?.classList.contains('col-help')?wrap.nextElementSibling:null;if(!help){help=document.createElement('div');help.className='col-help';wrap.after(help);}const label=th.textContent.replace('ⓘ','').trim();if(help.dataset.for===label){help.remove();return;}help.dataset.for=label;help.innerHTML=`<b>${esc(label)}</b> · ${esc(i.title)}`;return;}});
