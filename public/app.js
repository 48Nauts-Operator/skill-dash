...
const kindBadge = s => `<span class="mini-label">${s.kind==='plugin'?esc(s.plugin):s.kind==='repo'?'repo':s.imported_from?'imported':s.symlink?'linked':'own'}</span>`;
function filtered() {
...
function renderTable(actions,counts) {
 ...
 $('filters').innerHTML=[['all','All skills',s.length],['own','Own',s.filter(x=>x.kind==='own').length],['plugin','Plugin',s.filter(x=>x.kind==='plugin').length],['unused','Never invoked',counts.never],['flagged','⚠ Flagged',s.filter(x=>x.risk_flags?.length).length],...Object.keys(acolors).map(k=>[k,alabels[k],actions[k]]),['undecided','Undecided',counts.undecided],['decided','Decided',counts.decided],['pending','Pending',pending],...(errors?[['errors','Errors',errors]]:[])].map(([k,n,v])=>`<button class="filter ${filter===k?'active':''}" data-filter="${k}">${esc(n)}<em>${v}</em></button>`).join('');
 ...
 $('list-title').textContent=({all:'All skills',own:'Own skills',plugin:'Plugin skills',unused:'Never invoked',flagged:'Flagged by the static pre-scan',undecided:'Awaiting decision',decided:'Decided',pending:'Pending',errors:'Processing errors'}[filter]||alabels[filter]||filter);$('filtered-count').textContent=fmt(rows.length);
...
function radar(s) {
 ...
 const R=78,cx=110,cy=100,n=axes.length,ang=i=>-Math.PI/2+i*2*Math.PI/n,pt=(i,v)=>[cx+Math.cos(ang(i))*R*v,cy+Math.sin(ang(i))*R*v];
 ...
 const labels=axes.map(([label,v],i)=>{const [x,y]=pt(i,1.28);return `<text x="${x}" y="${y}" text-anchor="${Math.abs(Math.cos(ang(i)))<.2?'middle':Math.cos(ang(i))>0?'start':'end'}" dominant-baseline="middle" fill="#9aa5a0" font-size="8" font-family="ui-monospace,monospace">${esc(label.toUpperCase())} ${Math.round(v*100)}</text>`;}).join('');
 return `<div class="eyebrow">PROFILE</div><div class="judgment" style="padding:8px 6px 4px"><svg viewBox="0 0 220 200" role="img" aria-label="Radar of ${esc(s.id)} against the tree average" style="width:100%;max-width:330px;display:block;margin:auto">${[.25,.5,.75,1].map(ring).join('')}${axes.map((_,i)=>`<line x1="${cx}" y1="${cy}" x2="${pt(i,1)[0]}" y2="${pt(i,1)[1]}" stroke="#2b2622"/>`).join('')}${poly(mean,RADAR.mean,true)}${poly(axes.map(a=>a[1]),RADAR.skill,false)}${dots}${labels}</svg><p class="field-help" style="display:flex;gap:14px;justify-content:center;margin:4px 0 2px"><span><i style="display:inline-block;width:10px;height:2px;background:${RADAR.skill};vertical-align:middle;margin-right:5px"></i>this skill</span><span><i style="display:inline-block;width:10px;height:0;border-top:2px dashed ${RADAR.mean};vertical-align:middle;margin-right:5px"></i>tree average (${judged.length})</span></p></div>`;
}
