...
async function renderFind() {
 ...
 $('find-rows').innerHTML=rows.slice(0,60).map(x=>`<tr><td><a href="https://github.com/${esc(x.r)}/blob/${esc(x.c)}/${esc(x.p)}/SKILL.md"><b>${esc(x.n)}</b></a><br><small>${esc(x.r)}</small><br><span class="preview" style="white-space:normal">${esc(x.d)}</span></td><td>${scoreTag(x.gap)}</td><td>${scoreTag(x.fit)}<br><small>conf ${Math.round(x.fit_conf*100)}%</small></td><td class="confidence-cell">${Math.round(x.covered*100)}%</td><td style="max-width:320px;white-space:normal;font-size:11px;color:#bfb3a8">${x.matched?.length?`<b style="color:#d9cfc6">matched</b> ${x.matched.map(esc).join(', ')}<br>`:''}${x.closest_own?`<b style="color:#d9cfc6">closest of yours</b> ${esc(x.closest_own)} · ${Math.round(x.covered*100)}% twin`:''}</td><td>${x.flags?.length?`<span class="flag-badge" title="${esc(x.flags.join(', '))}">⚠ ${esc(x.flags.join(', '))}</span>`:'<small>clean</small>'}${x.j!=null?`<br><small>Jev risk ${x.j}</small>`:''}${x.cp?`<br><small>${x.cp} copies elsewhere</small>`:''}${x.dupes?`<br><small>${x.dupes} identical in the cut</small>`:''}</td><td><code style="font-size:10px;white-space:normal">git clone --depth 1 https://github.com/${esc(x.r)} /tmp/${esc(x.r.split('/')[1])} && cp -r /tmp/${esc(x.r.split('/')[1])}/${esc(x.p)} ~/.claude/skills/${esc(x.n)}</code></td></tr>`).join('')||'<tr><td colspan="7" class="empty"><strong>No search yet.</strong>Press Find skills. Nothing is written to your tree.</td></tr>';
}
$('find-button').onclick=async()=>{try{await api('/api/recommend',{k:Number($('find-k').value),with_prompts:$('find-prompts').checked,include_flagged:$('find-flagged').checked});await refresh();toast('Search started. Candidates are fetched from GitHub at their pinned commits, then read by Jev.');}catch(e){toast(e.message,true);}};
