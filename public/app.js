...
const kindBadge = s => `<span class="mini-label">${s.kind==='plugin'?esc(s.plugin):s.imported_from?'imported':s.symlink?'linked':'own'}</span>`;
function filtered() {
 ...
 let rows=data.skills.filter(s=>{
  ...
  switch(filter){
   ...
   case 'unused':return unused(s); case 'pending':return !s.result&&!s.error; case 'errors':return !!s.error;
   case 'undecided':return s.result&&!s.decision; case 'decided':return !!s.decision;
...
function render() {
 ...
 $('source-label').textContent=data.scan.status==='running'?`${total} skills · scanning transcripts…`:`${own} own · ${total-own} plugin · ${fmt(data.scan.transcripts||0)} transcripts`;
...
function renderTable(actions,counts) {
 ...
 $('filters').innerHTML=[['all','All skills',s.length],['own','Own',s.filter(x=>x.kind==='own').length],['plugin','Plugin',s.filter(x=>x.kind==='plugin').length],['unused','Never invoked',counts.never],...Object.keys(acolors).map(k=>[k,alabels[k],actions[k]]),['undecided','Undecided',counts.undecided],['decided','Decided',counts.decided],['pending','Pending',pending],...(errors?[['errors','Errors',errors]]:[])].map(([k,n,v])=>`<button class="filter ${filter===k?'active':''}" data-filter="${k}">${esc(n)}<em>${v}</em></button>`).join('');
 ...
 $('list-title').textContent=({all:'All skills',own:'Own skills',plugin:'Plugin skills',unused:'Never invoked',undecided:'Awaiting decision',decided:'Decided',pending:'Pending',errors:'Processing errors'}[filter]||alabels[filter]||filter);$('filtered-count').textContent=fmt(rows.length);
 $('rows').innerHTML=visible.map(x=>{const e=x.evidence;return `<tr><td class="check-cell"><input type="checkbox" data-select="${esc(x.id)}" aria-label="Select ${esc(x.id)}" ${selected.has(x.id)?'checked':''}></td><td class="sender"><div class="sender-wrap"><span class="sender-avatar">${esc(x.name.slice(0,2).toUpperCase())}</span><span>${esc(x.name)}</span></div></td><td><button class="subject-button" data-skill="${esc(x.id)}"><span class="subject-title">${esc(x.id)}${kindBadge(x)}</span><span class="preview">${esc(x.description.replace(/\s+/g,' '))}</span></button></td><td>${x.error?'<span class="tag" style="color:#ef8e8e;border-color:#7e4646">Error · retry</span>':scoreTag(useful(x))}</td><td>${x.error?'—':actionTag(rec(x))}</td><td class="confidence-cell">${e?`${fmt(e.invocations)}${e.slash?` <small>+${e.slash} slash</small>`:''}<br><small>${e.last_invoked?ago(e.last_invoked):e.script_mentions?`scripts ×${fmt(e.script_mentions)}`:'never'}</small>`:'<small>scanning…</small>'}</td><td class="confidence-cell">${x.overlap?`${x.overlap.score.toFixed(2)}<br><small>${esc(x.overlap.with)}</small>`:'—'}</td><td>${decisionTag(x.decision)}</td><td class="results-cell">${judgmentChips(x)}</td></tr>`;}).join('')||'<tr><td colspan="9" class="empty"><strong>Nothing here.</strong>Try another filter or search term.</td></tr>';
