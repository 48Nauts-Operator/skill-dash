...
FAQ = [
    ...
    ('A finding is wrong or outdated. How do I get a recheck?', 'Open an issue on github.com/48Nauts-Operator/whichskills-website with your repo name. We re-run the same pipeline against your current commit and update the row with the new commit hash. Free, and the old row stays visible with its date.'),
    ...
    ('Do you sell anything?', 'No. The data and the questions are public now, the pipeline follows as its own repository. The domain and the tokens are paid by 48Nauts.'),
]
...
<p>Two repositories. This site and its data files live at <a href="https://github.com/48Nauts-Operator/whichskills-website">github.com/48Nauts-Operator/whichskills-website</a>. The pipeline that produced them, the crawler, the static pre-scan, the Jev judge and the local dashboard, is a separate project (skill-dash) that we are preparing for release; when it is public, this paragraph will link to it and you can run the same pipeline on your own tree with <code>python3 server.py --roots &lt;dir&gt;</code>, where nothing leaves your machine except the Jev calls you choose to make. The verbatim Jev questions above are already the ones the pipeline sends. Data files: <a href="/data/report.json">report</a>, <a href="/data/clones.json">clones</a>, <a href="/data/jev-risk.json">jev-risk</a>, <a href="/data/corpus.json">corpus</a>, CC BY 4.0.</p></section>
